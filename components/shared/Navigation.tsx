...
export function Navigation() {
  ...
  const playerNavItems = [
    { label: 'Discover', href: '/player/discover', icon: Home },
  ...
  return (
    <div className="fixed bottom-5 left-0 right-0 z-50 md:hidden flex justify-center px-4 pointer-events-none">
      ...
      <nav className="pointer-events-auto bg-[var(--surface)] border border-[var(--card-border)] shadow-[0_12px_40px_rgba(0,0,0,0.3)] rounded-full px-2 py-1.5 flex items-center justify-between gap-1 max-w-[420px] w-full transition-all duration-300">
        {items.map((item) => {
          ...
          if (isActive) {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="bg-[var(--primary)] text-white px-4 py-2.5 rounded-full flex items-center gap-2 font-semibold text-xs tracking-tight shadow-md border border-[var(--primary-dark)] transition-all duration-300 shrink-0"
              >
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className="w-10 h-10 rounded-full flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text)] hover:bg-[var(--card)] transition-all duration-200"
              title={item.label}
